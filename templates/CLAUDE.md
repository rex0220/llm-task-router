...
- **再 factcheck は差分で要否判定**（二度手間回避）。初回 factcheck 後に `llm-task-router article:factcheck-stamp --accepted-after factcheck --note ...` で baseline を受理し、以降は `article:factcheck-scope` で `full|skip|diff` を判定（差分ゼロなら skip）。`factcheck-stamp` は信頼状態を変えるので factcheck 前に打たない（プロンプト維持）。**本文を変えたら（編集レビューの revise 含む）必ず `article:factcheck-scope` を回して判定を台帳に残す**（手動 skip で証跡を飛ばさない）。非事実差分なら `--accepted-after non-factual-diff` で stamp（`factcheck` は初回受理用）。
