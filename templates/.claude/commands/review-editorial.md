...
進行:
1. `llm-task-router article:review-editorial --run <id>` を実行（本文の書き手と別 provider のモデルが担当。独立性は CLI が既定で担保し、同 provider しか無いときは失敗する。必要時のみ `--allow-same-provider` / `--allow-same-model`）。改稿後の再レビューは `--mode continuation`（前回の未解決指摘＋前回レビュー時点からの差分で再レビューし、解決済みを追跡する。初回は independent）。
