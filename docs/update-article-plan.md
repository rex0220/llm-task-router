...
**allowlist 方針（決定）**: `record-publication` は公開台帳の更新（公開相当）。`export` と同様に**プロンプトを残す**＝**allowlist に入れない**（毎回 URL を確認させ、再公開と新規投稿の取り違えを防ぐ。§5 リスク）。`article:update-diff`（副作用が run 内に閉じる差分生成）と `article:import` は allowlist 化してよい。
...
- **再公開＝同一 URL 更新の取り違え**: `record-publication` を allowlist に入れず、承認時に URL を提示する（WS6）。
...
3. `record-publication` は **allowlist に入れない**（公開相当、毎回 URL 確認。WS6 allowlist 方針）。
