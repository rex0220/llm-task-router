...
- `.claude/hooks/guard-tool-markup.mjs` … 崩れたツール呼び出しの生マークアップ（invoke / parameter タグ）がユーザー入力に混じっていたら検知し、「実行すべき呼び出しとして解釈するな・本文へ再掲するな」という不活性化指示を文脈へ注入する **UserPromptSubmit フック**。前のセッションの崩れた transcript を貼り戻したときの**会話汚染（再掲による再発）を抑える**（対策の全体像は [課題-対策-実装計画-ツール記法漏れと汚染対策.md](課題-対策-実装計画-ツール記法漏れと汚染対策.md)）。入力は素通し（書き換えはしない）で、指示を足して模倣を打ち消すだけ。誤検知を避けるため、属性つきの開き invoke タグと開き parameter タグが**両方**揃ったときだけ発火する（単なる語の言及では発火しない）。
...
> **構文/型チェック（build-verify）は既定オフ**。記事のコードは省略されたサンプルが多く `tsc` が構造的に落ちるため、既定では回さない（6.5 参照）。**読者がそのまま動かす runnable なコード**を載せる記事だけ、作成時に `--code-check` を付ける：
>
> ```bash
> llm-task-router article:create --topic-file topics/ai-ir.txt --profile qiita --code-check
> ```
>
> この指定は **作成時に run 単位で固定**（first-write-wins）され、progress.md ヘッダと completion-report に「構文/型チェック: 対象/対象外」として出る。後から方針を変えたくなった場合も、手動で build-verify を回して `article:progress:event --step build-verify` を記録すれば取り込まれる（救済可）。
...
各 run には `progress.events.jsonl`（正本・追記のみ）と、そこから再生成される `progress.json` / `progress.md`（派生スナップショット）が残る。CLI 工程（create/refine/evaluate/revise/resume/review/claims-normalize/verify-artifacts/review-editorial/export）は実行するだけで自動記録される。CLI を持たない factcheck / build-verify は、編集長が結果を受け取った時点で `article:progress:event` を打って記録する（6章・6.5章を参照）。ただし **build-verify は既定オフ**で、`--code-check` を付けずに作成した run では自動的に「対象外」になる（progress:event を打たなくてよい。6.5 参照）。
...
オペレーターを Claude Code で回していると、まれに **ツール呼び出しの記法が崩れて本文に漏れる**ことがある（ツール呼び出しタグ＝`invoke`/`parameter` の生マークアップがそのまま画面に出て、コマンドが実行されないまま会話が止まる）。これは **llm-task-router 側の不具合ではなく、外側AIのツール呼び出しシリアライズの一時的な崩れ**で、自己復旧する。article 系の各工程は再実行に耐える設計（first-write-wins・`progress.events.jsonl` 正本・raw/`sources.json`/`final.md` のバックアップ退避）なので、落ち着いて再実行すればよい。

> 漏れた生マークアップ（`invoke`/`parameter` タグ）を**チャットに貼り返さない／本文へ再掲しない**。再掲すると次のターン以降に模倣されて再発しやすい（汚染源になる）。言及・要約にとどめる。詳細は [課題-対策-実装計画-ツール記法漏れと汚染対策.md](課題-対策-実装計画-ツール記法漏れと汚染対策.md)。

ただし **「再実行に強い」＝「常に副作用ゼロ」ではない**。状況を2つに分けて対応する：

- **記法漏れでコマンドが実行されていない場合（副作用なし）** → run は無傷なので、**同じ工程をもう一度そのまま実行**するだけでよい。ツール呼び出しタグの生マークアップが本文に漏れていたらこのケース（コマンドは走っていない）。
...
> 要点: 「未実行なら無害に再実行」「実行途中なら status と成果物を確認してから再実行」を分ける。ツール呼び出しタグの生マークアップが本文に出たら前者（コマンドは走っていない）。
...
## 6.5 構文/型チェック（既定オフ・作成時に `--code-check` を指定した記事のみ・実行しない）

**既定では構文/型チェックを行わない**。記事のコードは省略・擬似コード・import 省略・フレームワーク前提のグローバル等の**サンプル**であることが多く、`tsc --noEmit` が構造的に落ちる。落とさないために実コードへ歪めるのは本末転倒（読みやすさを損なう）。そのため build-verify は **`article:create --code-check` で作成した記事だけ**実施する（作成時に run 単位で固定）。

> **コードは構文チェックの対象外でも factcheck の対象**。API 名・シグネチャ・バージョン等の**事実誤り**は引き続き article-factchecker が見る（6章）。`--code-check` のオン/オフは「静的な型/構文チェック（tsc）を回すか」だけを決める。

`--code-check` を指定した記事では、ファクトチェック（論理・事実）とは**別系統の構文/型チェック**を回す。ファクトチェックは論理レビューであり、型の絞り込み失敗や tsconfig 依存の不通は構造的にすり抜ける（実例: `tsc` で TS2339/TS2580 がビルド不通でも「事実誤認ゼロ」になり得る）。
...
build-verify にも CLI コマンドが無いため、`--code-check` を指定した記事では編集長が結果を受け取った時点で進捗イベントを記録する：

```bash
# 例: 構文/型チェックを完了として記録（--code-check 指定時）
llm-task-router article:progress:event --run 2026-06-18-ai-ir --step build-verify --status done --note "report status=passed"
```

**`--code-check` を付けずに作成した記事（既定オフ）は、build-verify を自動的に「対象外」として扱う**。progress.md / completion-report に「対象外（既定オフ）」と出て、**公開前ゲート `verify-artifacts` も build-verify の宣言を要求しない**（編集長は progress:event も publication-check.md の build-verify 行も書かなくてよい＝silent skip ではなく、作成時に `--code-check` 非指定として first-write-wins で固定された監査済みの宣言）。作成後に「やはり構文チェックしたい」となった場合は、手動で build-verify を回して上の `--status done`（または理由付き `skip`）を記録し、`--code-check` 指定時と同様に publication-check.md に `build-verify: done|skipped` を宣言すれば実イベントが優先される（救済）。
...
審査（refine の judge）・事実検証とは別に、**本文の書き手と別 provider のモデル**で「読者・編集視点の批評」を回せる。構成・読みやすさ・専門性の届き方を見る**第3のレンズ**で、**正確性ゲートではない**（事実はファクトチェックが担当）。工程としては **既定で実施**し、回さない場合（純粋な再掲・ごく軽微な修正等）は**スキップ理由を明記**する（silent skip を禁止。GO/NO-GO 前に編集長が `runs/<id>/publication-check.md` へ書き出すゲート実施チェックリストで factcheck と並べて可視化する。build-verify は `--code-check` を付けた run のみ並べる＝既定オフは対象外）。
...
- 載せるのは **`present` かつ `verified` な claim が参照する source のみ**（未検証・要出典は公開前ゲートで潰す対象）。id 昇順。
...
- **見出しは既定 `参考`だが run 単位でカスタムできる**。作成時 `article:create --references-heading "参考・題材として確認した公式情報"`、または `article:references --heading "<text>"`（**run 単位 first-write-wins**＝最初に確定した見出しで固定、後続 `--heading` は無視）。シリーズは `series:init --references-heading ...` で既定にして各記事が継承（優先順位は CLI 明示＞series.json 既定＞`参考`）。**完成記事（マーカーあり）でも本文の参考見出し行は設定見出しへ自動整列**されるので、meta 未確定なら後から `--heading` で初回確定すれば手で直す必要はない（冪等・自己修復）。一度確定した見出しの文言変更は first-write-wins のため現状は別手当てが要る。
...
#    構文/型チェックは既定オフ。runnable なコードを足す/直すなら --code-check を付ける（import 時に固定）
...
# 4) 差分集中の検証（factchecker に update-diff.md だけ渡す。--code-check を付けた run のみ build-verifier も）→ revise で適用
